...
def apply_filter(img, k0, filter_type="hghp"):
    ...
    if (len(img.shape) < 3):
        return adjust_log(np.abs(apply_filter_gray(img, k0, filter_type = filter_type).astype(int)))
    else:
        return apply_filter_rgb(img, k0, filter_type = filter_type).astype(int)
    
image_cam = ski.data.camera()

image = ski.data.astronaut()

image_test = ski.io.imread('./PAL_Testbild_LowRFSignal.jpg')

image_frau = ski.io.imread('./Frau.jpg')

image_frau = rgb2gray(image_frau)

#image_cat = ski.data.cat()

plt.set_cmap('gray')
#plt.imshow(apply_filter(image_cam, 16, "hgtp"))
#plt.imshow(image[:, :, 0])
#img_verrauscht = apply_filter(image, 36, "hitp")

#plt.imshow(img_verrauscht)

#plt.imshow(apply_filter(image, 32, "hitp"))

plt.imshow(apply_filter(image, 32, "hitp"))
#plt.imshow(image_frau)